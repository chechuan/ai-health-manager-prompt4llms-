...
emotions_prompt = """你是一个非常专业的健康助手,请你协助我对用户的生命熵信息、睡眠信息、压力信息进行分析，以下是对分析内容的描述 我会提供近一周压力分级数据、睡眠质量状况, 请你根据自身经验分析：
1.分析我近一周的压力分级波动、分析整体压力高低、重度压力需要列出日期；
2.分析睡眠质量波动、分析睡眠质量好坏；
3.分析压力分级与睡眠质量的关系是正相关还是负相关，并解释原因。
只输出分析结果，每次输出不超过200字
压力分级数据： date 压力分级 2024/2/20 中度 2024/2/21 重度 2024/2/22 中度 2024/2/23 中度 2024/2/24 中度 2024/2/25 中度 2024/2/26 中度
睡眠质量分数在56-78之间
请遵循以下格式回复:
Thought: 思考拿到的是什么数据，针对当前数据应该做什么
Output: 结合以上信息,按要求给出分析。
