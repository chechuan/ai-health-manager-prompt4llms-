...
def parse_result(rsps_cache_path: Union[Path, str]):
    """解析结果记录"""
    result_save_path = rsps_cache_path.parent.joinpath(f"{rsps_cache_path.stem}_result_{args.model}.xlsx")
    rsps_result = [i for i in jsonlines.open(rsps_cache_path)]
    df = pd.DataFrame(
        columns=["原始问题", "是否查知识", "知识查询query", "引用知识", "最终回复"], index=range(len(rsps_result))
    )
...
async def main(query: str = "成人糖尿病夏天吃什么比较合适?"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as pool:
        ...
        for feature in concurrent.futures.as_completed(features):
            ...
            # if rsp:
            #     print(rsp)
    parse_result(rsps_cache_path)


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--threads", type=int, default=10, help="线程池并发量")
    parser.add_argument("--model", type=str, default="Qwen2-7B-Instruct", help="模型名称")
