...
class expertModel:
    ...
    async def health_blood_glucose_warning(self, param: Dict) -> str:
        ...
        prompt_vars = {"glucose_data": t+gl+result}
        sys_prompt = prompt_template.format(**prompt_vars)
    ...
    def health_blood_glucose_trend_analysis(self, param: Dict) -> str:
        ...
        if gl_code == "gl_2_pc":
            ...
            prompt_template_pc = (
                "# 任务描述\n"
                "# 你是一位经验丰富的医师助手，帮助医生对用户进行血糖管理，请你根据用户已知信息，一周的血糖情况，给医生提供专业的处理建议，只提供原则性的建议，包含5个方面：1.检查建议；2.饮食调整；3.运动调整；4.药物治疗；5监测血糖。建议的字数控制在120字以内\n\n"
                "# 已知信息\n"
                "## 用户信息\n"
                "年龄：{age}\n"
                "性别：{gender}\n"
