  ...
  "修改原因": "原运动方案中餐后仅建议散步15-30分钟，考虑到用户处于肥胖状态，为了更有效地管理体重和促进健康，增加了早间和晚间运动内容，进一步增加身体活动量，有助于提高基础代谢率和改善心肺功能。"
    ...
    async def aigc_functions_diet_recommendation(self, **kwargs):
        ...
        def get_bmi_desc(age, bmi):
            bmi = round(bmi, 1)  # 统一格式

            if age < 18:
                return f"暂不支持18岁以下人群的BMI解读。"

            if 18 <= age < 65:
                if bmi < 18.5:
                    return f"您的BMI（体质指数）是{bmi}，18.5—23.9属于适宜范围，您体重目前处于消瘦状态，消瘦不利于身体健康，容易出现营养不良、身体乏力、免疫力下降等情况，应注意均衡膳食、科学增重，早日达到健康体重状态。"
                elif 18.5 <= bmi < 24.0:
                    return f"您的BMI（体质指数）是{bmi}，18.5—23.9属于适宜范围，请继续保持。"
                elif 24.0 <= bmi < 28.0:
                    return f"您的BMI（体质指数）是{bmi}，18.5—23.9属于适宜范围，您体重目前处于超重状态，脂肪过多造成的BMI偏高，对于健康是不利因素，需要注意控制总热量摄入、增加体育锻炼，控制体重在正常范围内。"
                else:  # bmi ≥ 28
                    return f"您的BMI（体质指数）是{bmi}，18.5—23.9属于适宜范围，您体重目前处于肥胖状态，肥胖严重威胁身体健康，容易诱发糖尿病、心脏病、高血压、高血脂等多种疾病，建议您通过减少热量摄入、适量增加体育锻炼，把体重控制在健康状态。"
            else:  # age ≥ 65
                if bmi < 20:
                    return f"您的BMI（体质指数）是{bmi}，20—26.9属于适宜范围，您体重目前处于消瘦状态，消瘦不利于身体健康，容易出现营养不良、身体乏力、免疫力下降等情况，应注意食物品种丰富，合理膳食，早日达到健康体重状态。"
                elif 20 <= bmi < 26.9:
                    return f"您的BMI（体质指数）是{bmi}，20—26.9属于适宜范围，请继续保持。"
                elif 26.9 <= bmi < 28.0:
                    return f"您的BMI（体质指数）是{bmi}，20—26.9属于适宜范围，您体重目前处于超重状态，不要求您快速降低体重，而是应该维持在一个比较稳定的范围内。建议您通过合理膳食、适量增加体育活动，逐步将体重控制在健康状态。"
                else:  # bmi ≥ 28
                    return f"您的BMI（体质指数）是{bmi}，20—26.9属于适宜范围，您体重目前处于肥胖状态，不要求您快速降低体重，而是应该维持在一个比较稳定的范围内。建议您通过合理膳食、适量增加体育活动，逐步将体重控制在健康状态。"
