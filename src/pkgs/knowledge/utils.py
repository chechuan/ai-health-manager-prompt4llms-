...
from src.pkgs.knowledge.config.prompt_config import (
    PROMPT_TEMPLATES,
    SEARCH_QA_HISTORY_PROMPT,
    SEARCH_QA_PROMPT,
)
...
def parse_diff_pages(url: str, _html: str) -> (str, str):
    ...
    if "www.zhihu.com" in url:
        ...
        p_str_list = html.xpath(
            "//div[contains(@class, 'index_textContent ')]//p//text()"
        )
        text = "\n".join(p_str_list)
        ...
        if "/ec/article/" in url:
            title = html.xpath(
                '//*[@class="ArticleDetail_detailTitle__2ti6K undefined ArticleDetail_biggerTitle__3RZK1"]/text()'
            )[0]
            text = html.xpath('//*[@class="article-detail"]//text()')
        elif "/ec/question" in url:
            title = html.xpath('//*[@class="Title_detailTitle__3GfU7"]/text()')[0]
            text = html.xpath(
                '//*[@class="QuestionDetail_questionDetailWrapper__3QH5b"]//text()'
            )
    elif "songbai.cn" in url:
        if "pcs/qa/answers" in url:
            title = html.xpath(
                '//section[@class="qa-summary"]/h1[@class="title"]//text()'
            )[0]
        ...
        if "/ask/audio" in url:
            ...
            text = html.xpath(
                '//div[@class="audioData"]//div[@class="frame"]//p//text()'
            )
    else:
    ...
    **kwargs,
    ...
    if url.startswith("https://www.startpage.com/do/search?"):
        ...
        detail_urls = [
            i
            for j in [
                fetch_url_list(url + f"&page={page}&ie=utf8") for page in range(1, 3)
            ]
            for i in j
        ]
        detail_urls = [
            (i if i.startswith("http") else "https://www.sogou.com" + i)
...
class SearchQAChain(Chain):
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        return cls(qa_chain=qa_chain, ddg_search_chain=ddg_search_chain, **kwargs)
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        if not search_results:
            ...
            return return_vars
        _run_manager.on_text(
            f"search results num: {len(search_results)}",
            end="\n",
            color="green",
            verbose=self.verbose,
        )
        ...
        _run_manager.on_text(
            f"search qa answer: {qa_outputs['text']}",
            end="\n",
            color="green",
            verbose=self.verbose,
        )
