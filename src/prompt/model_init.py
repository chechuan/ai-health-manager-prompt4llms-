...
async def acallLLM(
    ...
    logger.trace(f"callLLM with {dumpJS(kwds)}")
    completion = None  # Initialize completion to None to avoid undefined reference errors
    retry = 0

    # Handle different input types based on whether there is history
    if not history:
        if "qwen" in model.lower():
            query = apply_chat_template(query)
        kwds["prompt"] = query

        # Retry mechanism for completions without history
        while retry <= retry_times:
            try:
                completion = await aclient.completions.create(**kwds)
                if completion.choices:  # Successfully received response
                    break
            except Exception as e:
                retry += 1
                logger.info(f"call llm error: {repr(e)}, retry attempt: {retry}")
        ...
        # Format history for chat model input
        if query:
            history += [{"role": "user", "content": query}]
        kwds["messages"] = history[-5:]  # Limit history to last 5 messages for efficiency

        # Retry mechanism for chat completions with history
        while retry <= retry_times:
            try:
    ...
    # Logging and time cost computation
    time_cost = round(time.time() - t_st, 1)
    if completion:
        logger.info(
            f"Model {model} generate costs summary: "
        ...
        logger.info("No valid completion generated.")
